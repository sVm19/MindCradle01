...
import { X, ShieldAlert, Loader2, Mail } from 'lucide-react';
...
function GoogleIcon({ className = '' }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <defs>
        <linearGradient id="mindcradle-google-gradient-modal" x1="2" y1="2" x2="22" y2="22" gradientUnits="userSpaceOnUse">
      ...
      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="url(#mindcradle-google-gradient-modal)"/>
      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="url(#mindcradle-google-gradient-modal)"/>
      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18A10.96 10.96 0 001 12c0 1.77.42 3.45 1.18 4.93l3.66-2.84z" fill="url(#mindcradle-google-gradient-modal)"/>
      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="url(#mindcradle-google-gradient-modal)"/>
    </svg>
  );
}

export default function AuthCardModal() {
  const { authModalOpen, setAuthModalOpen, setVerifyModalOpen, loginWithGoogleCode } = useAuth();
      ...
      <div className="w-full max-w-[390px] bg-bg2 border border-border rounded-[24px] px-8 py-8 relative shadow-2xl animate-slideIn">
        ...
        <div className="space-y-6">
          ...
          <Link
            to="/auth/magic-link"
            onClick={handleClose}
            className="w-full flex items-center justify-center gap-2 rounded-[14px] bg-bg3 border border-border2 px-4 py-3 text-sm font-medium text-text hover:bg-bg4 hover:border-white/25 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent transition-all"
          >
            <Mail size={16} />
          ...
          <div className="bg-bg3/40 border border-border rounded-xl p-3.5 text-[11px] text-text3 leading-relaxed text-center">
            MindCradle uses secure, passwordless authentication. Accounts are created instantly on first sign-in.
          </div>
        </div>
      </div>
    </div>
