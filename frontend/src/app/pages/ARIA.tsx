...
export default function ARIA() {
  ...
  const handleSearchHistory = async (query: string) => {
    ...
    try {
      const data = await aiApi.searchHistory(query);
  ...
  const handleClearChat = async () => {
    if (!window.confirm('Are you sure? This will delete all messages in this conversation.')) {
      return;
    }

    try {
      const response = await aiApi.clearChat();
      if (response) {
        clearARIAConversation();
        setSearchResults([]);
        setShowSearchResults(false);
  ...
  return (
    <div className="space-y-8 animate-fadeIn relative">
      ...
      <div className="relative mb-6">
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-text3 w-4 h-4" />
            <input
              type="text"
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  handleSearchHistory(searchQuery);
                }
              }}
              className="w-full bg-bg2 border border-border rounded-xl pl-11 pr-4 py-2.5 text-sm text-text placeholder:text-text3 focus:outline-none focus:border-accent/30"
            />
          </div>
          <button
            onClick={() => handleSearchHistory(searchQuery)}
            className="px-5 py-2.5 bg-accent hover:bg-accent2 text-white font-medium rounded-xl text-xs transition-all smooth-hover-btn"
          >
            🔍 Search
          </button>
          <button
            ...
            className="px-4 py-2.5 bg-bg3 hover:bg-bg4 border border-border text-text2 hover:text-text rounded-xl text-xs transition-all cursor-pointer"
          >
            ✕
          </button>
        </div>
      ...
      {/* Search Results */}
      {showSearchResults && (
        <div className="bg-bg2 border border-border rounded-2xl p-5 mb-6 space-y-4 animate-fadeIn">
          ...
          {searchResults.length > 0 ? (
            <div className="max-h-[300px] overflow-y-auto space-y-2 pr-2">
              {searchResults.map((result: any, idx: number) => (
                <div
                  key={idx}
                  className="bg-bg3 border border-border/50 hover:border-border rounded-xl p-4 transition-all"
                >
                  <p className="text-sm text-text leading-relaxed">{result.message}</p>
                  <div className="mt-2 flex justify-between items-center text-[10px] text-text3">
                    <span>
                    ...
                    {result.conversation_id && (
                      <span className="bg-bg4 px-2 py-0.5 rounded border border-border">
                        Convo #{result.conversation_id.slice(-6)}
                      </span>
                    )}
                  </div>
                </div>
            ...
            <p className="text-sm text-text3 py-4 text-center">No results found</p>
          )}
        </div>
      )}
