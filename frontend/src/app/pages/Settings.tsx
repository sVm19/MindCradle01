...
import { profile as profileApi, auth as authApi, user as userApi } from '@/lib/api';
...
export default function Settings() {
  ...
  const handleToggleAgeVerification = async () => {
    setIsChangingAge(true);
    setAgeError(null);
    const newStatus = !ageVerified;
    try {
      const res = await authApi.verifyAge(newStatus);
      setAgeVerified(res.verified);
      localStorage.setItem('age_verified', res.verified ? 'true' : 'false');
      if (res.verified) {
        const check = await authApi.checkAgeVerified();
        setAgeVerifiedAt(check.verified_at);
      } else {
        setAgeVerifiedAt(null);
      }
    } catch (err) {
      setAgeError(err instanceof Error ? err.message : 'Failed to update age verification.');
      setTimeout(() => setAgeError(null), 8000);
    } finally {
...
%%EOF`;
  ...
  const handleExportData = async () => {
    ...
    try {
      ...
      downloadAnchor.setAttribute("download", `mindcradle_user_data_${new Date().toISOString().slice(0, 10)}.json`);
  ...
  return (
    <>
      <div className="space-y-8 animate-fadeIn text-left">
        ...
        {/* MindCradle App (PWA Installation) */}
        <section className="bg-bg2 border border-border rounded-[20px] px-6 py-6 space-y-4">
          <div className="text-xs text-accent uppercase tracking-wider mb-2">MindCradle App</div>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <h3 className="text-sm font-semibold text-text">Install App for Offline Access</h3>
              <p className="text-xs text-text3 leading-relaxed max-w-xl">
                Install the app on your home screen for quick offline access and daily wellness rituals.
              </p>
            </div>
            <button
              onClick={handleInstallClick}
              className="px-5 py-2.5 bg-gradient-to-r from-[#E94B6F] to-[#f0739c] text-white hover:opacity-95 rounded-[10px] text-xs font-semibold flex items-center justify-center gap-1.5 transition-all shadow-md shadow-[#E94B6F]/20 active:scale-[0.98] cursor-pointer shrink-0"
            >
              <Download size={14} />
        ...
        <section className="space-y-4 text-left">
          ...
          >
            ...
            <div className="space-y-3.5">
              ...
              <div className="flex items-center justify-between gap-4 text-sm pt-2 border-t border-border/40">
                <div className="space-y-1">
                  <div className="text-text2 font-medium flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-3">
                  {ageError && (
                    <span className="text-xs text-rose font-medium animate-fadeIn max-w-[200px] text-right">
                      {ageError}
                    </span>
                  )}
                  <span className={`text-xs font-semibold uppercase tracking-wider ${ageVerified ? 'text-green' : 'text-text3'}`}>
                    {ageVerified ? 'Verified (18+)' : 'Under 18'}
                  </span>
                  <button
                    onClick={handleToggleAgeVerification}
                    disabled={isChangingAge}
                    className="px-3 py-1.5 bg-bg3 hover:bg-bg4 border border-border text-text2 hover:text-text rounded-lg text-xs font-semibold transition-all cursor-pointer"
                  >
                    {isChangingAge ? 'Updating...' : ageVerified ? 'Change to Under 18' : 'Verify as 18+'}
                  </button>
                </div>
              </div>
        ...
        <section className="bg-bg2 border border-rose/30 rounded-[20px] px-6 py-6 space-y-6">
          ...
          <div className="flex items-center justify-between gap-4 border-b border-border/40 pb-4">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-rose/10 hover:bg-rose/20 border border-rose/30 text-rose rounded-[10px] text-xs font-semibold transition-all cursor-pointer"
            >
              Sign out
            </button>
          </div>
          ...
          <div className="flex items-center justify-between gap-4 border-b border-border/40 pb-4">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-accent/15 hover:bg-accent/25 border border-accent/30 text-accent2 rounded-[10px] text-xs font-semibold transition-all cursor-pointer disabled:opacity-50"
            >
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <div className="text-sm text-rose font-medium">Delete Account (Permanent)</div>
              <div className="text-xs text-text3 mt-0.5">Permanently delete your account and all associated mood logs, journal entries, and conversations.</div>
            </div>
            <button
              onClick={() => {
                setDeleteError('');
              ...
              className="px-5 py-2.5 bg-rose/10 border border-rose/30 text-rose hover:bg-rose/20 rounded-[10px] text-xs font-semibold transition-all cursor-pointer"
            >
              Delete Account
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-md p-4 animate-fadeIn">
          <div className="w-full max-w-md bg-bg2 border border-rose/20 rounded-[24px] p-6 relative shadow-2xl animate-slideIn text-left space-y-5">
            ...
            <div className="flex items-center gap-3 border-b border-border pb-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-text">Delete Account & Data</h2>
      ...
      {showInstallInstructions && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-md p-4 animate-fadeIn">
          <div className="w-full max-w-md bg-bg2 border border-border rounded-[24px] p-6 relative shadow-2xl animate-scaleIn text-left space-y-5">
            <button
            ...
            <div className="flex items-center gap-3 border-b border-border pb-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-text">How to Install MindCradle</h2>
                <p className="text-xs text-text3">PWA Installation Guide</p>
              </div>
            </div>

            <div className="space-y-4 text-xs md:text-sm text-text2 leading-relaxed">
              <p>
                To install MindCradle on your device and use it like a native app:
              </p>
              
              <div className="space-y-3">
                <div className="bg-bg3 border border-border rounded-xl p-3">
                  <span className="font-semibold text-text block mb-1">📱 iOS (Safari)</span>
                  Tap the <strong className="text-accent">Share</strong> button in the bottom menu bar, scroll down, and select <strong className="text-accent">Add to Home Screen</strong>.
                </div>
                
                <div className="bg-bg3 border border-border rounded-xl p-3">
                  <span className="font-semibold text-text block mb-1">🤖 Android (Chrome)</span>
                  Tap the <strong className="text-accent">Menu</strong> button (three dots) at the top-right, and select <strong className="text-accent">Install app</strong> or <strong className="text-accent">Add to Home Screen</strong>.
                </div>

                <div className="bg-bg3 border border-border rounded-xl p-3">
                  <span className="font-semibold text-text block mb-1">💻 Desktop (Chrome/Edge)</span>
                  Click the <strong className="text-accent">Install icon</strong> in the right-side of the browser address bar, or open the browser menu and select <strong className="text-accent">Install MindCradle</strong>.
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <button
                type="button"
                onClick={() => setShowInstallInstructions(false)}
                className="px-5 py-2 bg-bg3 border border-border text-text2 hover:text-text rounded-xl text-xs font-semibold hover:bg-bg4 transition-all cursor-pointer"
              >
                Close
              </button>
            </div>
